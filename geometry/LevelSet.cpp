...
template<int d> void LevelSet<d>::Set_By_Geom(ImplicitGeometry<d>& geom)
{
    iterate_cell(iter, grid) {
        ...
        phi(cell) = geom.Phi(pos);}
}

template<int d> real LevelSet<d>::Phi(const VectorD& pos) const {return intp->Interpolate_Centers(phi,pos);}

template<int d> Vector<real,d> LevelSet<d>::Normal(const VectorD& pos) const
{
    VectorD normal;for(int i=0;i<d;i++)
        normal[i]=(Phi(pos+VectorD::Unit(i)*grid.dx)-Phi(pos-VectorD::Unit(i)*grid.dx))/((real)2*grid.dx);
