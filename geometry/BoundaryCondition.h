...
#ifndef SIMPLEXOLDCLION_BOUNDARYCONDITION_H
#define SIMPLEXOLDCLION_BOUNDARYCONDITION_H
...
enum class CellType: ushort {Fluid=0,Air,Solid,IB,Source};
...
public:
    ...
    real Psi_N_Value(const int axis,const VectorDi& face)
    {return psi_N_values[Vector2i(axis,mac_grid.Face_Index(axis,face))];}
    ushort Psi_D_Type(const VectorDi& cell)
    {return psi_D_values[mac_grid.grid.Cell_Index(cell)].first;}
    real Psi_D_Value(const VectorDi& cell)
    {return psi_D_values[mac_grid.grid.Cell_Index(cell)].second;}
