...
#ifndef SIMPLEXOLDCLION_POINTSET_H
#define SIMPLEXOLDCLION_POINTSET_H
...
#include "NeighborKDTree.h"
#include "NeighborHashing.h"
...
public:
    ...
    KernelSPH<d> v_kernel;										////volumetric SPH kernel
    KernelSPH<d-1> t_kernel;									////tangential SPH kernel
...
public:
    ...
    {
        if(_points==nullptr&&points==nullptr){points=new GeometryParticles<d>();own_data=true;}
        else{if(_points!=nullptr){delete points;points=_points;own_data=false;}}
        ...
        v_kernel.Initialize(v_r);
        t_kernel.Initialize(t_r);
    ...
    ////Tangential Neighbor
    inline const Array<int>& Tangential_Neighbor_Of(int idx) { return tang_nbs[idx]; }
    ...
    ////2D->1D projection
    inline void Project_To_TPlane(const Vector2& u, const Matrix2& e, Vector1& t) const { t = Vector1(u.dot(e.col(0))); }
    ////3D->2D projection
    inline void Project_To_TPlane(const Vector3& u, const Matrix3& e, Vector2& t) const { t = Vector2(u.dot(e.col(0)), u.dot(e.col(1))); }
    ////2D->1D projection including the local h-axis
    inline void Project_To_TPlane(const Vector2& u, const Matrix2& e, Vector2& t) const { t = Vector2(u.dot(e.col(0)), u.dot(e.col(1))); }
    ////3D->2D projection including the local h-axis
    inline void Project_To_TPlane(const Vector3& u, const Matrix3& e, Vector3& t) const { t = Vector3(u.dot(e.col(0)), u.dot(e.col(1)), u.dot(e.col(2))); }

    ////2D->1D projection for h in the normal direction
    inline real Project_To_TPlane_H(const Vector2& u, const Matrix2& e) const { return u.dot(e.col(1)); }
    ////3D->2D projection for h in the normal direction
    inline real Project_To_TPlane_H(const Vector3& u, const Matrix3& e) const { return u.dot(e.col(2)); }
    ...
    template<typename F> VectorT Grad_TPlane_SPH(const int i,const F& phi)
    {
        ...
        for(int k=0;k<nbs_num;k++){
            ...
            VectorT grad=phi(j)*Vol(j)*t_kernel.Grad_Spiky(lr_ij);
            grad_phi+=grad;}
        return grad_phi;
    }
    ...
    template<typename F> real Laplacian_TPlane_SPH(const int i,const F& phi)
    {
        ...
        for(int k=0;k<nbs_num;k++){
            ...
            if(i==j) continue;
            real lap=-2*Vol(j)*phi_ij*t_kernel.Grad_Cubic(lr_ij).norm()/sqrt(lr2);
    ...
    template<typename F> VectorT Laplacian_VecT_TPlane_SPH(const int i,const F& u)
    {
        ...
        for(int k=0;k<nbs_num;k++){
            ...
            if(i==j) continue;
            VectorT lap_ij=-2*Vol(j)*u_ij*t_kernel.Grad_Cubic(lr_ij).norm()/lr_ij.norm();//sqrt(lr2);
            lap_u+=lap_ij;}
        return lap_u;
    }
    ...
    template<typename F> VectorX Fit_Local_MLS(const int p,const F& phi){
        ...
        for (size_t i = 0; i < n; i++) {
            int nb = tang_nbs[p][i];
            if (nb == p) pi = (int)i;
            VectorD th; Project_To_TPlane(points->X(nb) - x_p, local_frame, th);
        ...
        while(t<max_distance){
            ...
            while(t<max_distance){
                ...
                if(cp_idx<0){//// if find no cp, move v_r forward
                    t+=v_r;} //// WH: I guess v_r<radius in cp searching?
            ...
            while(true){
                //// compute intersection
                {
                    ...
                    VectorD lp; Project_To_TPlane(tmp_p-proj_p,lf,lp);
                    VectorD ld; Project_To_TPlane(ray_d,lf,ld);
