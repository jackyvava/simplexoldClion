...
#ifndef SIMPLEXOLDCLION_KRYLOVSOLVER_H
#define SIMPLEXOLDCLION_KRYLOVSOLVER_H
#include <iostream>
#include "Common.h"
#include "Sparse.h"
#include <Eigen/Dense>
...
template<class KA> class KrylovPrecDiagonal
{
public:
    const KA* ka = nullptr;
    Eigen::VectorXd inv;

    KrylovPrecDiagonal(const KA* _ka) :ka(_ka) { Initialize(); }
    void Initialize() {
        inv.resize(ka->cols());
        for (int j = 0; j < ka->outerSize(); ++j)
        {
            typename KA::InnerIterator it(*ka, j);
            while (it && it.index() != j) ++it;
            if (it && it.index() == j && it.value() != 0.0)
                inv(j) = 1.0 / std::abs(it.value());
            else
                inv(j) = 1.0;
        }
    }
    template<class KXW> void Precond(const KXW& r, KXW& rst) {
        *rst.kx = inv.cwiseProduct(*r.kx);
    }
};
...
namespace KrylovSolver
{
    ...
    inline bool CG(const SparseMatrix<real>& A, VectorN<real>& x, const VectorN<real>& b, const Params params = Params())
    {
        ...
        KrylovPrecDiagonal<SparseMatrix<real> > kP(&A);
    ...
    //////////////////////////////////////////////////////////////////////////
    ////GMRES
    inline bool GMRES(const SparseMatrix<real>& A, VectorN<real>& x, const VectorN<real>& b, const Params params = Params());
