...
#ifndef SIMPLEXOLDCLION_PARTICLES_H
#define SIMPLEXOLDCLION_PARTICLES_H
#include <memory>
#include <map>
#include <set>
#include <fstream>
...
template<class T> void Register_Att(std::map<std::string,ArbitraryArrayPointer>& att_map,Array<std::string> &reverse_names,ArrayPtr<T> &att)
{att_map[reverse_names.back()]=ArbitraryArrayPointer(att);reverse_names.pop_back();}
template<typename T, typename... Args>
void Register_Att(std::map<std::string,ArbitraryArrayPointer> &att_map,Array<std::string> &reverse_names,ArrayPtr<T> &att,Args & ...rest)
{Register_Att<T>(att_map,reverse_names, att);Register_Att(att_map,reverse_names, rest...);}
...
public:
    std::map<std::string, ArbitraryArrayPointer> att_map;
