...
namespace KrylovSolver {
    ...
    bool GMRES(const SparseMatrix<real>& A, VectorN<real>& x, const VectorN<real>& b, const Params params)
    {
        ...
        MatrixXd V(MatrixXd::Zero(N, M + 1));	// orthogonal vectors for Arnoldi iteration
        MatrixXd H(MatrixXd::Zero(M + 1, M));	// Hessenberg matrix for Arnoldi iteration

        Eigen::ArrayXd C(Eigen::ArrayXd::Zero(M + 1));	// cos of given rotations
        Eigen::ArrayXd S(Eigen::ArrayXd::Zero(M + 1));	// sin of given rotations
        ...
        for (int iter = 0; iter < params.max_iter_num; iter++)
        {
            ...
            VectorXd g(VectorXd::Zero(M + 1));	// vectorg = ||r|| * e1
            g(0) = r_n;
