...
#ifndef SIMPLEXOLDCLION_GEOMETRYPRIMITIVES_H
#define SIMPLEXOLDCLION_GEOMETRYPRIMITIVES_H
#include <limits>
#include <iostream>
#include "Common.h"
#include "AuxFunc.h"
...
public:
    ImplicitGeometry(){}
    virtual real Phi(const VectorD& pos) const {return (real)0;}
    virtual bool Inside(const VectorD& pos) const {return Phi(pos)<(real)0;}
    virtual VectorD Normal(const VectorD& pos) const {return VectorD::Zero(); std::cout << "hello\n";}
};
...
public:
    ...
    virtual real Phi(const VectorD& pos) const {
        VectorD diff;
        diff = (pos - center);
        if (diff.dot(-1 * bowl_normal) >= 0.) {
            return diff.dot(-1*bowl_normal);
        }
        else{ return radius - (pos - center).norm(); }
    }
    virtual VectorD Normal(const VectorD& pos) const { return (center - pos).normalized(); }
};
